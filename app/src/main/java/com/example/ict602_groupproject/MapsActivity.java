...
import com.example.ict602_groupproject.databinding.ActivityMapsBinding;
    ...
    public Response.Listener<String> onSuccess = new Response.Listener<String>() {
        ...
        public void onResponse(String response) {
            ...
            if (informations.length <1){
                Toast.makeText(getApplicationContext(),"Problem retriving data",Toast.LENGTH_LONG).show();
                return;
            }
            for (Information info : informations){
                Double lat = Double.parseDouble(info.lat);
                Double lng = Double.parseDouble(info.lng);
                String title = info.reporterName + " reported " + info.typeHazard;
                String snippet = info.timeDateReported;
